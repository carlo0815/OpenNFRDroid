...
import org.opennfr.opennfrdroid.helpers.enigma2.requesthandler.DeviceInfoRequestHandler;
...
public class CheckProfile {
	...
	public static ExtendedHashMap checkProfile(Profile profile, Context context) {
		...
		OpenNFRDroid.disableSleepTimer();
		OpenNFRDroid.disableNowNext();
						...
						if (dirh.parse(xml, deviceInfo)) {
							...
							if (vc >= 0) {
								...
								if (checkVersion(version, requiredForSleeptimer) >= 0)
									OpenNFRDroid.enableSleepTimer();
								if (checkVersion(version, FEATURE_EPGNOWNEXT_VERSION) >= 0)
									OpenNFRDroid.enableNowNext();
								if (checkVersion(version, FEATURE_POST_REQEUEST) >= 0)
									OpenNFRDroid.setFeaturePostRequest(true);
								else
									OpenNFRDroid.setFeaturePostRequest(false);
