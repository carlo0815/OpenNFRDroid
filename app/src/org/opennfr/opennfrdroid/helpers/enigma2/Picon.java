...
public class Picon {
	...
	public static String getBasepath(Context context){
		...
		if(sp.getBoolean(OpenNFRDroid.PREFS_KEY_PICONS_ONLINE, OpenNFRDroid.isTV(context))) {
			return String.format("%s/", sp.getString(OpenNFRDroid.PREFS_KEY_SYNC_PICONS_PATH, "/usr/share/enigma2/picon"));
		}

		if(!Environment.getExternalStorageDirectory().canWrite())
			return String.format("%s%spicons%s", context.getFilesDir().getAbsolutePath(), File.separator, File.separator);

		return String.format("%s%sopennfrDroid%spicons%s", Environment.getExternalStorageDirectory()
	...
	public static String getPiconFileName(Context context, ExtendedHashMap service, boolean useName) {
		...
		if(PreferenceManager.getDefaultSharedPreferences(context).getBoolean(OpenNFRDroid.PREFS_KEY_FAKE_PICON, false))
	...
	public static void setPiconForView(Context context, ImageView piconView, ExtendedHashMap service, String tag, Callback callback) {
		...
		SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
		if (!sp.getBoolean(OpenNFRDroid.PREFS_KEY_PICONS_ENABLED,
				OpenNFRDroid.isTV(context))) {
			piconView.setVisibility(View.GONE);
			return;
		}
		boolean useName = sp.getBoolean(OpenNFRDroid.PREFS_KEY_PICONS_USE_NAME, false);
		...
		Picasso.get().load(uri).fit().centerInside().tag(tag).error(R.drawable.opennfrdroid_logo_simple).into(piconView, callback);
	}
	...
	public static String getPiconUri(Context context, String fileName) {
		...
		if(PreferenceManager.getDefaultSharedPreferences(context).getBoolean(OpenNFRDroid.PREFS_KEY_PICONS_ONLINE, OpenNFRDroid.isTV(context))) {
