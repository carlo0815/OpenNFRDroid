...
import org.opennfr.opennfrdroid.util.Base64;
...
public class SimpleHttpClient {
	...
	public String buildEncoderStreamUrl(String ref) {
		if (mProfile.getHost().equals("opennfr.tech"))
	...
	public boolean fetchPageContent(String uri, List<NameValuePair> parameters) {
		...
		try {
			...
			if (OpenNFRDroid.featurePostRequest())
			...
			if (conn.getResponseCode() != 200) {
				if (conn.getResponseCode() == HttpURLConnection.HTTP_BAD_METHOD && mRememberedReturnCode != HttpURLConnection.HTTP_BAD_METHOD) {
					...
					OpenNFRDroid.setFeaturePostRequest(!OpenNFRDroid.featurePostRequest());
					conn.disconnect();
	...
	private void dumpToFile(URL url) {
		...
		String base = String.format("%s/opennfrDroid/xml", externalStorage);
	...
	public void applyConfig() {
		if (mProfile == null)
			mProfile = OpenNFRDroid.getCurrentProfile();
