...
public class AsyncListLoader extends AsyncTaskLoader<LoaderResult<ArrayList<ExtendedHashMap>>> {
	...
	public LoaderResult<ArrayList<ExtendedHashMap>> loadInBackground() {
		...
		if (mRequireLocsAndTags) {
			if (OpenNFRDroid.getLocations().size() <= 1) {
				if (!OpenNFRDroid.loadLocations(mShc)) {
					Log.e(OpenNFRDroid.LOG_TAG, "ERROR loading locations");
				}
			}

			if (OpenNFRDroid.getTags().size() <= 1) {
				if (!OpenNFRDroid.loadTags(mShc)) {
					Log.e(OpenNFRDroid.LOG_TAG, "ERROR loading tags");
				}
			}
		}
