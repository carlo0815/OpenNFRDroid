...
public class MyPreferenceFragment extends PreferenceFragmentCompat implements
	...
	public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
		...
		if (OpenNFRDroid.PREFS_KEY_THEME_TYPE.equals(key)) {
			updateThemeSummary();
			OpenNFRDroid.setTheme((AppCompatActivity) getActivity());
		} else if (OpenNFRDroid.PREFS_KEY_HWACCEL.equals(key))
			updateHwAccelSummary(prefs);
	}
	...
	protected void updateThemeSummary() {
		if (getActivity() == null)
			return;
		int idx = OpenNFRDroid.getThemeType(getActivity());
		Preference themePref = findPreference(OpenNFRDroid.PREFS_KEY_THEME_TYPE);
	...
	protected void updateHwAccelSummary(SharedPreferences prefs) {
		if (getActivity() == null)
			return;
		int idx = Integer.parseInt(prefs.getString(OpenNFRDroid.PREFS_KEY_HWACCEL, Integer.toString(VLCPlayer.MEDIA_HWACCEL_ENABLED)));
		Preference themePref = findPreference(OpenNFRDroid.PREFS_KEY_HWACCEL);
