...
import org.opennfr.opennfrdroid.DatabaseHelper;
import org.opennfr.opennfrdroid.OpenNFRDroid;
import org.opennfr.opennfrdroid.Profile;
import org.opennfr.opennfrdroid.R;
import org.opennfr.opennfrdroid.adapter.recyclerview.ServiceAdapter;
import org.opennfr.opennfrdroid.adapter.recyclerview.SimpleExtendedHashMapAdapter;
import org.opennfr.opennfrdroid.adapter.recyclerview.SimpleTextAdapter;
import org.opennfr.opennfrdroid.fragment.abs.BaseHttpRecyclerEventFragment;
import org.opennfr.opennfrdroid.fragment.dialogs.EpgDetailBottomSheet;
import org.opennfr.opennfrdroid.fragment.helper.HttpFragmentHelper;
...
import org.opennfr.opennfrdroid.helpers.SyncService;
...
import org.opennfr.opennfrdroid.helpers.enigma2.requesthandler.AbstractListRequestHandler;
import org.opennfr.opennfrdroid.helpers.enigma2.requesthandler.EpgNowNextListRequestHandler;
import org.opennfr.opennfrdroid.helpers.enigma2.requesthandler.EventListRequestHandler;
...
import org.opennfr.opennfrdroid.widget.AutofitRecyclerView;
...
public class ServiceListFragment extends BaseHttpRecyclerEventFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (mDetailReference == null) {
			mDetailReference = OpenNFRDroid.getCurrentProfile().getDefaultRef();
			mDetailName = OpenNFRDroid.getCurrentProfile().getDefaultRefName();
		}

		if (mNavReference == null) {
			mNavReference = OpenNFRDroid.getCurrentProfile().getDefaultRef2();
			mNavName = OpenNFRDroid.getCurrentProfile().getDefaultRef2Name();
		}
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		((AutofitRecyclerView) mDetailList).setMaxSpanCount(
				Integer.parseInt(
						prefs.getString(
								OpenNFRDroid.PREFS_KEY_GRID_MAX_COLS,
	...
	public void onPrepareOptionsMenu(Menu menu) {
		...
		String defaultReference = OpenNFRDroid.getCurrentProfile().getDefaultRef();
	...
	public Loader<LoaderResult<ArrayList<ExtendedHashMap>>> onCreateLoader(int id, Bundle args) {
		...
		if (id == LOADER_BOUQUETLIST_ID || mPickMode) {
			...
			if (OpenNFRDroid.featureNowNext())
	...
	public void showPopupMenu(View v) {
		...
		menu.getMenu().findItem(R.id.menu_next_event).setVisible(OpenNFRDroid.featureNowNext());
	...
	public void reload(String ref, boolean isBouquetList) {
		...
		if (isBouquetList) {
			...
			if (OpenNFRDroid.checkInitial(getAppCompatActivity(), OpenNFRDroid.INITIAL_SERVICELIST_PANE)) {
				mSlidingPane.openPane();
				OpenNFRDroid.setNotInitial(getAppCompatActivity(), OpenNFRDroid.INITIAL_SERVICELIST_PANE);
			} else {
				mSlidingPane.closePane();
			}
			String param = "bRef";
