...
import org.opennfr.nfrdroid.helpers.enigma2.requesthandler.DeviceInfoRequestHandler;
...
public class CheckProfile {
	...
	public static ExtendedHashMap checkProfile(Profile profile, Context context) {
		...
		nfrDroid.disableSleepTimer();
		nfrDroid.disableNowNext();
						...
						if (dirh.parse(xml, deviceInfo)) {
							...
							if (vc >= 0) {
								...
								if (checkVersion(version, requiredForSleeptimer) >= 0)
									nfrDroid.enableSleepTimer();
								if (checkVersion(version, FEATURE_EPGNOWNEXT_VERSION) >= 0)
									nfrDroid.enableNowNext();
								if (checkVersion(version, FEATURE_POST_REQEUEST) >= 0)
									nfrDroid.setFeaturePostRequest(true);
								else
									nfrDroid.setFeaturePostRequest(false);
