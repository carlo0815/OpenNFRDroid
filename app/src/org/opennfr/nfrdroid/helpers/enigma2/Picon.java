...
public class Picon {
	...
	public static String getBasepath(Context context){
		...
		if(sp.getBoolean(nfrDroid.PREFS_KEY_PICONS_ONLINE, nfrDroid.isTV(context))) {
			return String.format("%s/", sp.getString(nfrDroid.PREFS_KEY_SYNC_PICONS_PATH, "/usr/share/enigma2/picon"));
		}
	...
	public static String getPiconFileName(Context context, ExtendedHashMap service, boolean useName) {
		...
		if(PreferenceManager.getDefaultSharedPreferences(context).getBoolean(nfrDroid.PREFS_KEY_FAKE_PICON, false))
	...
	public static void setPiconForView(Context context, ImageView piconView, ExtendedHashMap service, String tag, Callback callback) {
		...
		SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
		if (!sp.getBoolean(nfrDroid.PREFS_KEY_PICONS_ENABLED,
				nfrDroid.isTV(context))) {
			piconView.setVisibility(View.GONE);
			return;
		}
		boolean useName = sp.getBoolean(nfrDroid.PREFS_KEY_PICONS_USE_NAME, false);
		...
		Picasso.get().load(uri).fit().centerInside().tag(tag).error(R.drawable.nfrdroid_logo_simple).into(piconView, callback);
	}
	...
	public static String getPiconUri(Context context, String fileName) {
		...
		if(PreferenceManager.getDefaultSharedPreferences(context).getBoolean(nfrDroid.PREFS_KEY_PICONS_ONLINE, nfrDroid.isTV(context))) {
