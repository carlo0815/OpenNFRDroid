...
import org.opennfr.nfrdroid.adapter.recyclerview.ServiceAdapter;
import org.opennfr.nfrdroid.adapter.recyclerview.SimpleExtendedHashMapAdapter;
...
import org.opennfr.nfrdroid.fragment.helper.HttpFragmentHelper;
import org.opennfr.nfrdroid.helpers.ExtendedHashMap;
import org.opennfr.nfrdroid.helpers.NameValuePair;
import org.opennfr.nfrdroid.helpers.RecyclerViewPauseOnScrollListener;
import org.opennfr.nfrdroid.helpers.Statics;
import org.opennfr.nfrdroid.helpers.SyncService;
...
import org.opennfr.nfrdroid.helpers.enigma2.requesthandler.AbstractListRequestHandler;
import org.opennfr.nfrdroid.helpers.enigma2.requesthandler.EpgNowNextListRequestHandler;
...
import org.opennfr.nfrdroid.widget.AutofitRecyclerView;
...
public class ServiceListFragment extends BaseHttpRecyclerEventFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (mDetailReference == null) {
			mDetailReference = nfrDroid.getCurrentProfile().getDefaultRef();
			mDetailName = nfrDroid.getCurrentProfile().getDefaultRefName();
		}

		if (mNavReference == null) {
			mNavReference = nfrDroid.getCurrentProfile().getDefaultRef2();
			mNavName = nfrDroid.getCurrentProfile().getDefaultRef2Name();
		}
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		((AutofitRecyclerView) mDetailList).setMaxSpanCount(
				Integer.parseInt(
						prefs.getString(
								nfrDroid.PREFS_KEY_GRID_MAX_COLS,
	...
	public void onPrepareOptionsMenu(Menu menu) {
		...
		String defaultReference = nfrDroid.getCurrentProfile().getDefaultRef();
	...
	public void showPopupMenu(View v) {
		...
		menu.getMenu().findItem(R.id.menu_next_event).setVisible(nfrDroid.featureNowNext());
	...
	public void reload(String ref, boolean isBouquetList) {
		...
		if (isBouquetList) {
			...
			if (nfrDroid.checkInitial(getAppCompatActivity(), nfrDroid.INITIAL_SERVICELIST_PANE)) {
				mSlidingPane.openPane();
				nfrDroid.setNotInitial(getAppCompatActivity(), nfrDroid.INITIAL_SERVICELIST_PANE);
			} else {
				mSlidingPane.closePane();
			}
			String param = "bRef";
