...
public class MyPreferenceFragment extends PreferenceFragmentCompat implements
	...
	public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
		...
		if (nfrDroid.PREFS_KEY_THEME_TYPE.equals(key)) {
			updateThemeSummary();
			nfrDroid.setTheme((AppCompatActivity) getActivity());
		} else if (nfrDroid.PREFS_KEY_HWACCEL.equals(key))
			updateHwAccelSummary(prefs);
	}
	...
	protected void updateThemeSummary() {
		if (getActivity() == null)
			return;
		int idx = nfrDroid.getThemeType(getActivity());
		Preference themePref = findPreference(nfrDroid.PREFS_KEY_THEME_TYPE);
	...
	protected void updateHwAccelSummary(SharedPreferences prefs) {
		if (getActivity() == null)
			return;
		int idx = Integer.parseInt(prefs.getString(nfrDroid.PREFS_KEY_HWACCEL, Integer.toString(VLCPlayer.MEDIA_HWACCEL_ENABLED)));
		Preference themePref = findPreference(nfrDroid.PREFS_KEY_HWACCEL);
