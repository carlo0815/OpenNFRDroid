...
import org.opennfr.nfrdroid.helpers.DateTime;
import org.opennfr.nfrdroid.helpers.SimpleHttpClient;
import org.opennfr.nfrdroid.helpers.enigma2.requesthandler.LocationListRequestHandler;
import org.opennfr.nfrdroid.helpers.enigma2.requesthandler.TagListRequestHandler;
 ...
 * @author sre
 */
public class nfrDroid extends MatomoApplication {
	private static volatile nfrDroid instance;
	...
	public static final String ACTION_CREATE = "nfrdroid.intent.action.NEW";
	public static final String LOG_TAG = "nfrDroid";
	...
	public static Context getAppContext() {
		if (instance != null) return instance;
			...
			try {
				instance = (nfrDroid) Class.forName("android.app.ActivityThread").getDeclaredMethod("currentApplication").invoke(null);
	...
	private void initChannels() {
		...
		NotificationChannel channel = new NotificationChannel("nfrdroid_picon_sync",
				getString(R.string.picons),
				NotificationManager.IMPORTANCE_LOW);
		channel.setDescription(getString(R.string.sync_picons));
		notificationManager.createNotificationChannel(channel);

		channel = new NotificationChannel("nfrdroid_epg_sync",
	...
	public static void loadCurrentProfile(Context context) {
		...
		if (profiles.isEmpty()) {
			...
			Profile p = new Profile(-1, "Open NFR", host, streamHost, port, 8001, 80, login, user, pass, ssl, false, false,
					false, false, "", "", "", "");
			dbh.addProfile(p);
			profileId = p.getId();
		...
		if (!setCurrentProfile(context, profileId)) {
			...
			sProfile = new Profile(-1, "Open NFR", "192.168.1.2", "", 80, 8001, 80, false, "", "", false, false, false, false,
					false, "", "", "", "");
		}
	}
	...
	public static boolean setCurrentProfile(Context context, int id, boolean forceEvent) {
		...
		if (sProfile != null) {
			...
			Log.w(nfrDroid.LOG_TAG, "no profile with given id [" + id + "] found");
		}
		return false;
	}
